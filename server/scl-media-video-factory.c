#include "scl-media-video-factory.h"
#include "scl-media.h"

G_DEFINE_TYPE(SclMediaVideoFactory, scl_media_video_factory, GST_TYPE_RTSP_MEDIA_FACTORY)

GST_DEBUG_CATEGORY_STATIC(scl_media_video_factory);
...
static void scl_media_factory_init(SclMediaVideoFactory* self) {
    // gst_rtsp_media_factory_set_media_gtype(GST_RTSP_MEDIA_FACTORY(self), SCL_TYPE_MEDIA);
}

static void scl_media_factory_class_init(SclMediaVideoFactoryClass* klass) {
    ...
    GST_DEBUG_CATEGORY_INIT(scl_media_video_factory, "sclmediavideofactory", 0, "SclMediaVideoFactory");
}
